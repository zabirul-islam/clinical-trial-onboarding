...
\subsection{Zero lexical cross-trial leakage with residual semantic leakage quantified}
...
\begin{table}[!htbp]
\centering
\caption{\textbf{Adversarial-subset stratification of the 456 open-weight case-by-backbone cells.} The adversarial subset was defined pre hoc as dominance ratio $\rho < 1.5$ and at least two trials in the candidate pool. Lexical detectors flagged zero leakage in both strata. The semantic detector's both-judges-agree rate was approximately $3$--$5\times$ higher in the adversarial subset than in the non-adversarial subset and was highest for Qwen-2.5-3B. Either-judge rates are shown in parentheses.}
...
This pattern is consistent with the design intent of the gate ensemble. Lexical leakage remained absent in the adversarial subset, whereas the semantic detector localized residual paraphrase leakage to cases that were structurally difficult for single-trial selection. The strongest residual was observed for Qwen-2.5-3B ($5.7\%$ adversarial vs.\ $1.6\%$ non-adversarial). These results suggest two possible mitigation strategies for future deployment-oriented evaluation: raising the abstention threshold for cases with $\rho < 1.5$, or adding the post-hoc semantic-redaction step discussed under Theorem~\ref{thm:safety}.
...
\emph{Fifth, baseline parity is incomplete.} The closed-API baselines, GPT-4o and Claude Sonnet~4.5, are evaluated zero-shot with the same guarded prompt and evidence. A stronger baseline comparison would evaluate published trial-matching systems within their own full pipelines or apply the proposed guard to their retrieved evidence and patient-facing outputs. We leave this to future work because the output type and available artifacts differ across systems.

\emph{Sixth, semantic leakage is not yet removed in the render path.} The two lexical detectors flagged $0/684$ generations, but the semantic LLM-judge detector flagged $12/684$ generations ($1.75\%$) under both-judges-agree consensus. Manual review classified eight of these as unambiguous paraphrased cross-trial content, two as mixed-attribution errors, and two as likely judge over-flags. Because the current patient-facing render path does not remove these paraphrased semantic errors, an NLI postprocessor that drops sentences not entailed by the accepted evidence $\mathcal{E}^\star$ is a recommended hardening step, especially for the smallest open-weight backbone, Qwen-2.5-3B, where the residual was concentrated in the adversarial subset.
