...
def main():
    ...
    index = faiss.read_index(str(IND / f"dense_index__{tag}.faiss"))
    doc_ids = np.load(IND / f"dense_doc_ids__{tag}.npy", allow_pickle=True)
    ...
    run_rows = []
    metric_rows = []
    run_name = f"dense__{tag}"

    for qid, score_row, idx_row in tqdm(zip(qids, scores, indices), total=len(qids), desc=run_name):
        ranked_docs = []
    ...
    run_df.to_csv(OUT / f"dense_run__{tag}.csv", index=False)
    metrics_df.to_csv(OUT / f"dense_per_query_metrics__{tag}.csv", index=False)
    pd.DataFrame([summary]).to_csv(OUT / f"dense_summary_metrics__{tag}.csv", index=False)

    print("Saved:")
    print(OUT / f"dense_run__{tag}.csv")
    print(OUT / f"dense_per_query_metrics__{tag}.csv")
    print(OUT / f"dense_summary_metrics__{tag}.csv")
    print("\nSummary:")
