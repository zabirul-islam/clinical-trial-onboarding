...
def main() -> int:
    ...
    p.add_argument("--cands-csv", type=str, default="bm25_full_text_top100_candidates.csv",
                   help="Candidates CSV inside outputs/tables/ (e.g. bm25_full_text_top1000_candidates.csv)")
    p.add_argument("--out-suffix", type=str, default="",
                   help="Suffix appended to output filenames, e.g. '_top1000pool'")
    args = p.parse_args()

    root = args.repo_root.resolve()
    table_dir = root / "outputs/tables"
    cands_path = table_dir / args.cands_csv
    ...
    sfx = args.out_suffix
    pd.DataFrame(out_rows).to_csv(table_dir / f"bge_reranker_large_run{sfx}.csv", index=False)
    per_q_df = pd.DataFrame(per_q)
    per_q_df.to_csv(table_dir / f"bge_reranker_large_per_query_metrics{sfx}.csv", index=False)
    ...
    summ.insert(3, "pool", args.cands_csv)
    summ.to_csv(table_dir / f"bge_reranker_large_summary_metrics{sfx}.csv", index=False)
