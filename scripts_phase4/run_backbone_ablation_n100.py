...
def main() -> int:
    ...
    p.add_argument("--no-gate", action="store_true",
                   help="skip selector accept-gate; treat all cases as accepted")
    args = p.parse_args()
    ...
    if args.no_gate:
        accept = pd.Series([True] * len(cache), index=cache.index)
    else:
        accept = (
    ...
    for bb in args.backbones:
        ...
        for _, r in tqdm(pick.iterrows(), total=len(pick), desc=bb.split('/')[-1]):
            ...
            slug = bb.replace("/", "__")
            gen_dir = root / "outputs/phase4/n100_expansion/gens" / slug
    ...
    out_dir = root / "outputs/phase4/n100_expansion"
    out_dir.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_dir / "backbone_ablation_n100_raw.csv", index=False)
    ...
    agg.to_csv(out_dir / "backbone_ablation_n100_summary.csv", index=False)
